 ...
 `).insertBefore('.nav-tabs');$("div#tlogins button.btn.btn-primary").click(function(){var myVar=setInterval(function(){var usmsgw=$(".pmsgc").length;if(usmsgw>0){$(`
    <div class="uzr fl corner borderg mm" style="border-radius:5px;margin-bottom:-2px;width:99.5%;padding:0px; background-color:white;">
      ...
      <div class="uzr fl" style="padding:0px;width:80%">
        ...
        <div style="padding: 0px;width: 100%;color: rgb(60, 0, 255);text-align: left;display: block;margin-top: -17px;" class=" u-msg   break  fl"> ترحيب شات قمر الشام<h1 style="display: inline-block;color: red;">`+getuser(myid).topic+`</h1>   شات قمر الشام </div> </div>
    </div></div>
    `).appendTo('div#d2')
...
><font style="background-color: #fff;border-radius: 0px 25px 0px 25px;font-family: 'Cairo', sans-serif, FontAwesome;padding: 2px 10px 2px 10px; margin:0px 25px 0px 25px;" >مـرحبـاً بكم في شـات  قمر الـشام للجوال</font></div></div><center>`).insertBefore('#d2');$(`<img style="height:50px;width: 100%;margin-right: 5px;background-color: #DE9DA5;" src="https://www.chatjawwal.com/bnnr.jpeg" border="0">`).insertBefore('div#d2bc');$('.break.light').css({"background-image":"url(https://chatjawwal.com/bnr.jpg)"});(document.getElementById("tbox").placeholder="احمد تجربة");$("div#tlogins button.btn.btn-primary").click(function(){var c=setInterval(function(){var a=$(".pmsgc").length;0<a?($(`<img style="height: 99.3%;
width:99%;
margin-top: 2px;
pointer-events:none;
 margin-bottom: 5px;
...
`).insertBefore('span.s1.fa.fa-user.label.badgex.label-as-badge.label-success');$(".fa-gear").text("الاعدادات).css("width","");$(`<label onclick="window.open('cp?token='+token,'_blank');" style="background-color: ghostwhite;color: black;margin:4px; padding:8px;width:98%;" class="label tc border cp  btn  label-danger fl"><span class="fl fa fa-star"></span>مـرحبـاً بكم في شـات  قمر الـشام </label>`).insertAfter('#settings label.label.tc.border.btn.fl[onclick="sendpic();"]');Tsend_=Tsend;reqlikes=200;Tsend=function(){if(getuser(myid).rep<reqlikes){alert('مـرحبـاً بكم في شـات  قمر الـشام للجوال'+reqlikes+' ');return;}else{Tsend_();}}
